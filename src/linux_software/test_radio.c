...
void print_benchmark(volatile unsigned int *periph_base)
{
    ...
    const unsigned int reads = 2048;
    const double bytes_transferred = (double)reads * 4.0;  // 32-bit register
    const double f_clk = 125e6; // timer clocks at 125 MHz
    unsigned int delta = stop_time - start_time;
    const double seconds = ((double)delta) / f_clk;
    const double mb_per_s = (bytes_transferred/1.0e6) / seconds;
    printf("You transferred %.0f bytes of data in %.9f seconds\n", bytes_transferred, seconds);
    printf("Measured Transfer throughput = %.3f Mbytes/sec\n", mb_per_s);
}
...
// first, get a pointer to the peripheral base address using /dev/mem and the function mmap
    ...
    printf("\r\n\r\n\r\nLab 10 Alejandro Brito - Custom Peripheral Demonstration\n\r");
